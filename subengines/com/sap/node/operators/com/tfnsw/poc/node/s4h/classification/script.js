...
const { ClfnClassForKeyDate, ClfnClassDescForKeyDate } = require('@sap/cloud-sdk-op-vdm-class-data-for-classification-service');
...
const createClass = async (classData) => {
    ...
    if (batch ) {
        op.logger.info(`Create Batch call for Classification`);

        
    } else {
        try {
            op.logger.info(`Create Single call for Classification`);
            // const classification = ClfnClassForKeyDate.builder()
            //     .class(classData.class)
            //     .classType(classData.classType)
            //     .toClassDescription([
            //         ClfnClassDescForKeyDate.builder()
            //             .language('en')
            //             .classDescription(classData.toClassDescription.classDescription)
            //             .build()
            //     ])
            //     .build();
            op.logger.info(classData)
            const classification = ClfnClassForKeyDate.builder()
                .fromJson(classData);
            op.logger.info(classification)
        
            const newResp = await ClfnClassForKeyDate.requestBuilder()
                ...
                .catch(err => {
                    ...
                    op.getOutPort("error").send({
                        Attributes: { "message.request.id": classData["class"] },
                        Encoding: 'string',
                        Body: {
                            "error": err.message || {},
            ...
            if (newResp) {
                let message = {
                    Attributes: { "message.request.id": newResp['class']  },
                    Encoding: 'string',
                    Body: newResp
                };
